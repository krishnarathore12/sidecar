---
name: codex
description: Use PROACTIVELY as the default subagent for implementation, debugging, bug fixing, code review, and test writing — prefer delegating these to this agent instead of doing them yourself or using general-purpose agents. Runs GPT 5.5 at xhigh reasoning effort via the OpenAI Codex CLI on the user's ChatGPT subscription.
...
1. Run the task non-interactively, always with GPT 5.5 at xhigh reasoning effort:

   ```bash
   codex exec -m gpt-5.5 -c model_reasoning_effort="xhigh" --sandbox workspace-write --skip-git-repo-check "<full task description>"
   ```
